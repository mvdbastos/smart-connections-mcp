...
export class GitManager {
  ...
  private getUnavailableSyncResult(): GitSyncResult | null {
    if (!this.isGitAvailable()) {
      return {
        success: false,
        branch: '',
  ...
  private getUnavailablePushResult(): GitPushResult | null {
    if (!this.isGitAvailable()) {
      return {
        success: false,
        branch: '',
  ...
  private getCommitArgs(message: string, authorName?: string, authorEmail?: string): string[] {
  ...
  private getBehindRemote(branch: string): number {
    return this.getAheadBehind(branch).behindRemote;
  }

  private getRemoteCommits(branch: string, limit: number): GitSyncResult['commits'] {
    if (limit <= 0) {
      return [];
    }
