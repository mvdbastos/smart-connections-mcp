...
function resource(slug: string, name: string, description: string, text: string): MemoryResource {
  return {
...
export const MEMORY_RESOURCES: MemoryResource[] = [
  resource(
    'index',
    'Memory MCP Workflow Guide',
    'Overview of the note workflow and available operations.',
    `# Memory MCP Workflow
...
Notes are embedded on create/edit for semantic search. See **embeddings** resource.`
  ...
  resource(
    'tools',
    'Tool Reference',
    'Parameters and usage for all available tools.',
    `# Tool Reference
...
- \`git_sync_notes\` → use \`get_stats\` to monitor sync state`
  ...
  resource(
    ...
    'How idle-debounced synchronization works.',
    `# Sync Scheduler
...
- \`error\`: Last commit/push failure if any`
  ...
  resource(
    ...
    'How note embeddings work and fallback behavior.',
    `# Embeddings
