 ...
 */
function createVaultWithStaleSources(indexed, onDisk) {
    ...
    const lines = indexed.map((notePath) => `${JSON.stringify(`smart_sources:${notePath}`)}: ${JSON.stringify(source(notePath))},`);
    fs.writeFileSync(path.join(vault, '.smart-env', 'multi', 'sources.ajson'), `${lines.join('\n')}\n`);
    return vault;
}
