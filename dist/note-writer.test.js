...
describe('editNote refuses to fabricate notes', () => {
    it('throws for every edit mode when the file does not exist', () => {
        ...
        try {
            ...
            expect(() => editNote(vault, 'Missing.md', { mode: 'replace', content: 'x', find: 'y' })).toThrow(/not found/i);
            expect(() => editNote(vault, 'Missing.md', { mode: 'insert-after-heading', content: 'x', heading: 'H' })).toThrow(/not found/i);
        }
        finally {
            fs.rmSync(vault, { recursive: true, force: true });
        }
    ...
    it('refuses a dry run against a missing note too', () => {
        const vault = fs.mkdtempSync(path.join(os.tmpdir(), 'w-'));
        try {
            expect(() => editNote(vault, 'Missing.md', { mode: 'append', content: 'x', dryRun: true })).toThrow(/not found/i);
        }
        finally {
            fs.rmSync(vault, { recursive: true, force: true });
        }
